...
(function(){
  ...
  if (!el) return;
  const messages = [
    "神经网络是一个拥有数百万个可调节旋钮（权重）的函数。训练就是转动这些旋钮，直到函数的输出与它看到的示例相匹配。",
    "GPT 生成的每个词都是通过与你这里构建的相同的前向传播计算出来的——只是有 4050 亿个权重而不是 19 个。",
    "反向传播不是魔法：它是递归应用的链式法则。每个权重都会被告知它对误差的贡献有多大。",
  ...
  function render() {
    ...
    Object.values(nodes).forEach(node => {
      ...
      if (showVals) {
        ...
        html += `<text x="${node.x}" y="${node.y + 8}" text-anchor="middle" font-family="JetBrains Mono" font-size="9" fill="#C8D0DC">值=?</text>`;
      }
      ...
      if (showGrads) {
        ...
        html += `<text x="${node.x}" y="${node.y + 38}" text-anchor="middle" font-family="JetBrains Mono" font-size="9" fill="#C8D0DC">梯度=?</text>`;
      }
    });

    // Legend
    html += `<text x="10" y="245" font-family="Inter" font-size="10" fill="#697386">a=2, b=-3. 前向传播: 乘法 → 加法 → tanh.</text>`;
    if (showGrads) {
