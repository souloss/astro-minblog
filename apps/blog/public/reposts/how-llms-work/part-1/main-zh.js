...
(function(){
  ...
  const messages = [
    "它是一个大型语言模型——一个拥有 4050 亿参数的神经网络，被训练来预测序列中的下一个 Token...",
    "魔法在于权重：15 万亿 Token 的训练数据被压缩成编码人类知识的参数...",
    "你看到的每个词都是从 100,277 个可能 Token 的概率分布中采样得到的。没有魔法——只有大规模的数学。"
  ];
...
(function(){
  ...
  const steps = [
    { label: '原始文本', tokens: ['t','o','k','e','n','i','z','a','t','i','o','n'], desc: '起始：单个字符' },
    { label: '字节 (UTF-8)', tokens: ['116','111','107','101','110','105','122','97','116','105','111','110'], desc: '编码为字节 ID (0–255)' },
    { label: 'BPE 合并 1 后', tokens: ['to','k','en','i','za','ti','on'], desc: '合并频繁对' },
    { label: 'BPE 合并 2 后', tokens: ['token','iza','tion'], desc: '继续合并' },
    { label: '最终 Token', tokens: ['token','ization'], desc: 'GPT-4：该词为 2 个 Token' }
  ];
  let step = 0;
  ...
  function renderStep() {
    ...
    html += `<text x="190" y="185" text-anchor="middle" font-family="Barlow Condensed" font-size="11" fill="#697386" letter-spacing="1">点击前进</text>`;
...
(function(){
  ...
  const layers = [
    ...
    { label: 'Token 嵌入', color: '#946800', y: 272 },
    { label: '多头注意力', color: '#635BFF', y: 220 },
    { label: '前馈网络 (MLP)', color: '#0570DE', y: 168 },
...
(function(){
  ...
  const trainingTexts = {
    ...
    500: `<span class="incoherent">模型已学</span><span class="coherent">习</span> <span class="incoherent">但混</span><span class="coherent">乱</span> <span class="incoherent">仍然 wqp</span> <span class="coherent">模型</span> <span class="incoherent">bns</span> <span class="coherent">去预测</span>...`,
    5000: `<span class="coherent">语言模型学会了预测序列中的下一个 Token。</span> <span class="incoherent">训练需要数千</span> <span class="coherent">梯度下降步</span> <span class="incoherent">才能收敛</span>。`,
    32000: `<span class="coherent">神经网络已经学会了人类语言的统计模式。它几乎可以生成任何主题的连贯文本、回答问题、编写代码，并进行多轮对话。</span>`
  };
  ...
  sampleBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        infoEl.textContent = `采样到 "${processed[selectedIdx].t}"（概率 ${(processed[selectedIdx].prob * 100).toFixed(1)}%）`;
...
(function() {
  ...
  function draw() {
    ...
    ctx.fillText('colony / capital 簇', .06*W, .75*H);
    ctx.fillStyle = 'rgba(123,110,102,0.3)';
    ctx.fillText('science 簇', .55*W, .52*H);
    if (showLines) {
    ...
    timings.forEach((t, i) => {
      setTimeout(() => {
        ...
        if (i === 5) {
          ...
          runBtn.disabled = false; runBtn.textContent = '▶ 再次运行';
        }
      }, t);
    });
  }
