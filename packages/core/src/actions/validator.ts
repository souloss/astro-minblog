...
function validateScrollToSection(payload: unknown): { valid: true } | { valid: false; error: string } {
  if (!isObject(payload)) {
    return { valid: false, error: 'payload must be an object' };
  }

  if (typeof payload.sectionId !== 'string' || payload.sectionId.trim() === '') {
    return { valid: false, error: 'sectionId is required and must be a non-empty string' };
  }

  if (payload.behavior !== undefined && !['smooth', 'instant'].includes(payload.behavior as string)) {
  ...
  if (payload.highlight !== undefined && typeof payload.highlight !== 'boolean') {
    return { valid: false, error: 'highlight must be a boolean' };
  }

  if (payload.highlightDuration !== undefined && typeof payload.highlightDuration !== 'number') {
...
function validateHighlightText(payload: unknown): { valid: true } | { valid: false; error: string } {
  if (!isObject(payload)) {
    return { valid: false, error: 'payload must be an object' };
  }
  ...
  if (payload.style !== undefined && !['accent', 'warning', 'info', 'success'].includes(payload.style as string)) {
    return { valid: false, error: 'style must be "accent", "warning", "info", or "success"' };
  }
  ...
  if (payload.scrollIntoView !== undefined && typeof payload.scrollIntoView !== 'boolean') {
...
function validateToggleTheme(payload: unknown): { valid: true } | { valid: false; error: string } {
  if (!isObject(payload)) {
    return { valid: false, error: 'payload must be an object' };
  }
...
function validateToggleReadingMode(payload: unknown): { valid: true } | { valid: false; error: string } {
  if (!isObject(payload)) {
    return { valid: false, error: 'payload must be an object' };
  }
...
function validateSetPreference(payload: unknown): { valid: true } | { valid: false; error: string } {
  if (!isObject(payload)) {
    return { valid: false, error: 'payload must be an object' };
  }
...
function validateNavigate(payload: unknown): { valid: true } | { valid: false; error: string } {
  if (!isObject(payload)) {
    return { valid: false, error: 'payload must be an object' };
  }
...
const VALIDATORS: Record<ActionType, (payload: unknown) => { valid: true } | { valid: false; error: string }> = {
...
export function validateAction(raw: unknown): { valid: true; action: Action } | { valid: false; error: string } {
  if (!isObject(raw)) {
  ...
  if (!isValidType(type)) {
    return { valid: false, error: `invalid type: ${JSON.stringify(type)}. Valid types: ${[...VALID_TYPES].join(', ')}` };
  }
  ...
  if (!result.valid) {
    return { valid: false, error: `payload validation failed: ${result.error}` };
  }
...
export function validateAndExecute(action: unknown): Promise<ActionResult> {
  ...
  const executor = (window as unknown as { __actionExecutor?: { execute: (a: Action) => Promise<ActionResult> } }).__actionExecutor;

  if (!executor) {
    return Promise.resolve({ success: false, error: 'ActionExecutor not initialized' });
  }
