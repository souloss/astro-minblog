...
import { resolveAnswerMode, getCitationGuardPreflight, createCitationGuardTransform, buildUnknownRefusal } from '../intelligence/citation-guard.js';
...
describe('resolveAnswerMode', () => {
  ...
  describe('NOT unknown mode (precise pattern matching)', () => {
    it.each([
      ...
      ['storage 怎么用？', 'general'],
      ['介绍一下 page 功能', 'fact'],
    ])('should NOT detect "%s" as unknown mode (avoid false positives)', (query, expected) => {
      expect(resolveAnswerMode(query)).toBe(expected);
    });
  });
  ...
  describe('general mode (fallback)', () => {
    it.each([
      ...
      ['hello'],
      ['好的'],
    ])('should detect "%s" as general mode', (query) => {
...
describe('getCitationGuardPreflight', () => {
  const mockArticles: ArticleContext[] = [
    { title: 'Test Article', url: '/test', keyPoints: [], summary: 'Test summary', categories: [], dateTime: Date.now() },
  ];
  ...
  describe('article count queries', () => {
    it('should return count for "有几篇文章" queries', () => {
      const articles: ArticleContext[] = [
        { title: 'A', url: '/a', keyPoints: [], summary: '', categories: [], dateTime: Date.now() },
        { title: 'B', url: '/b', keyPoints: [], summary: '', categories: [], dateTime: Date.now() },
        { title: 'C', url: '/c', keyPoints: [], summary: '', categories: [], dateTime: Date.now() },
      ];
      const result = getCitationGuardPreflight({
        userQuery: '有几篇文章？',
        articles,
...
describe('createCitationGuardTransform', () => {
  const mockArticles: ArticleContext[] = [
    { title: 'Test', url: '/posts/test', keyPoints: [], categories: [], dateTime: Date.now() },
  ];
  const mockProjects: ProjectContext[] = [
    { name: 'Project', url: 'https://example.com/project', description: 'Test project' },
  ];

  async function transformStream(input: string, articles: ArticleContext[], projects: ProjectContext[], siteUrl?: string): Promise<string> {
    const guard = createCitationGuardTransform({ articles, projects, siteUrl });
    ...
    const outputStream = guard(inputStream);
  ...
  it('should allow valid article links', async () => {
    const input = 'Check out [Test Article](/posts/test) for more info.';
    const result = await transformStream(input, mockArticles, [], 'https://example.com');
    expect(result).toBe(input);
  });

  it('should allow valid project links', async () => {
    const input = 'See [Project](https://example.com/project) for details.';
    const result = await transformStream(input, [], mockProjects);
    expect(result).toBe(input);
  });

  it('should remove hallucinated external links', async () => {
    const input = 'Check [Fake Article](https://fake-site.com/article) for more.';
    const result = await transformStream(input, mockArticles, []);
    expect(result).toBe('Check Fake Article for more.');
  });

  it('should allow relative internal links with siteUrl', async () => {
    const input = 'Read [Test](/posts/test) for details.';
    const result = await transformStream(input, mockArticles, [], 'https://example.com');
    expect(result).toBe(input);
  });

  it('should handle multiple links in one chunk', async () => {
    const input = 'See [Valid](/posts/test) and [Fake](https://fake.com/page).';
    const result = await transformStream(input, mockArticles, [], 'https://example.com');
    expect(result).toBe('See [Valid](/posts/test) and Fake.');
  });

  it('should preserve text without links', async () => {
