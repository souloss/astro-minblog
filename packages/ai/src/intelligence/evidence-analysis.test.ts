...
} from "../intelligence/evidence-analysis.js";
...
describe("shouldSkipAnalysis", () => {
  ...
  it("should skip for fewer than 2 articles", () => {
    expect(shouldSkipAnalysis("This is a moderate query", 1, "moderate")).toBe(
      true
    );
  });
  ...
  it("should not skip for moderate complexity with enough articles", () => {
    expect(
      shouldSkipAnalysis(
        "This is a longer query text for analysis",
        3,
        "moderate"
      )
    ).toBe(false);
  ...
  it("should not skip for complex queries", () => {
    expect(
      shouldSkipAnalysis(
        "This is a complex query that needs detailed analysis",
        5,
        "complex"
      )
    ).toBe(false);
