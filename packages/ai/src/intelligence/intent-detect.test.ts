...
import { classifyIntent, rankArticlesByIntent } from "../query/intent.js";
import {
  isLikelyFollowUp,
...
describe("rankArticlesByIntent", () => {
  ...
  it("should return original order for general intent", () => {
    const articles = [createArticle("First"), createArticle("Second")];
...
describe("isLikelyFollowUp", () => {
  ...
  it("should not detect long messages as follow-up", () => {
    expect(
      isLikelyFollowUp(
        "我想了解一下关于 Astro 框架的详细信息，包括它的特点和优势"
      )
    ).toBe(false);
    expect(
      isLikelyFollowUp(
        "这是一段很长的消息，超过了四十八个字符的限制，所以不应该被识别为追问"
      )
    ).toBe(false);
  });
...
describe("hasNewSignificantTokens", () => {
  ...
  it("should return true when tokens exist that are not in cached query", () => {
    expect(hasNewSignificantTokens("React components Vue", "React hooks")).toBe(
      true
    );
  });
