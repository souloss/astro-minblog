...
import type { CitationGuardPreflight } from "./types.js";
import {
...
export function buildUnknownRefusal(
  query: string,
  lang: string = "zh"
): string {
  const normalized = query.trim().toLowerCase();
  ...
  if (
    /(老婆叫什么|妻子叫什么|丈夫叫什么|孩子叫什么|父母叫什么|家人姓名|家人)/u.test(
      normalized
    )
  ) {
...
export function getCitationGuardPreflight(params: {
  ...
  if (/有几篇|有多少篇|文章数量|总共.*文章|how many.*article/u.test(q)) {
    ...
    if (total > 0) {
      const text = pickTemplateWithVars(ARTICLE_COUNT_TEMPLATES, lang, {
        count: total,
      });
