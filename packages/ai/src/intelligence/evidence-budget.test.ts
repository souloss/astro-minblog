...
} from "../intelligence/evidence-budget.js";
...
describe("applyBudgetToArticles", () => {
  ...
  const mockArticles: ArticleContext[] = [
    createArticle("A", "This is a long summary that should be truncated", [
      "1",
      "2",
      "3",
      "4",
  ...
  it("should remove full content when disabled", () => {
    const articlesWithContent = mockArticles.map(a => ({
      ...a,
      fullContent: "long content...",
    }));
    const budget = getEvidenceBudget("simple");
