...
export const toggleThemeTool = tool({
  description:
    "Toggle the blog theme between light, dark, and system modes. Use this when the user wants to change the visual theme of the blog.",
...
export const navigateToArticleTool = tool({
  description:
    "Navigate to a specific blog article by its slug. Use this when the user wants to open or jump to a specific article.",
  inputSchema: z.object({
    slug: z
      .string()
      .describe(
    ...
    lang: z
      .enum(["zh", "en"])
      .optional()
      .describe('Optional: The language of the article (defaults to "zh")'),
  }),
});

export const scrollToSectionTool = tool({
  description:
    "Scroll to a specific section within the current article. Use this when the user wants to jump to a chapter or section.",
  inputSchema: z.object({
    sectionId: z
      .string()
      .describe(
    ...
    behavior: z
      .enum(["smooth", "instant"])
      .optional()
      .describe('Scroll behavior (default: "smooth")'),
  }),
});
...
export const highlightTextTool = tool({
  description:
    "Highlight specific text in the article. Use this when the user wants to emphasize or mark text.",
  inputSchema: z.object({
    ...
    style: z
      .enum(["accent", "warning", "info", "success"])
      .optional()
      .describe('Highlight style (default: "accent")'),
    duration: z
      .number()
      .optional()
      .describe(
    ...
    scrollIntoView: z
      .boolean()
      .optional()
      .describe("Whether to scroll the highlighted element into view"),
  }),
});

export const setPreferenceTool = tool({
  description:
    "Set a user preference value. Use this when the user wants to customize their experience.",
  inputSchema: z.object({
    key: z
      .string()
      .describe('The preference key (e.g., "theme", "reading.fontSize")'),
...
export const searchArticlesTool = tool({
  description:
    'Search for blog articles and projects matching a query. Use this when the user wants to find specific content or when they ask to navigate to a topic (e.g., "AI module", "architecture"). Returns matching articles with titles, URLs, and summaries.',
  inputSchema: z.object({
    ...
    includeProjects: z
      ...
      .optional()
      .default(true)
      .describe("Whether to include projects in search results"),
  }),
  execute: async ({
    query,
    limit,
...
const builtinTools: Record<string, AnyTool> = {
...
export function getClientSideTools(): string[] {
  return [
    ...
    ...Object.keys(customTools).filter(
      k => !(customTools[k] as { execute?: unknown }).execute
    ),
  ];
}
