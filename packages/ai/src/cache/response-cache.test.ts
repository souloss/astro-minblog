...
import { buildResponseCacheKey, getResponseCache, setResponseCache } from './response-cache.js';
...
describe('buildResponseCacheKey', () => {
  it('isolates response cache keys by articleSlug and lang', () => {
    expect(buildResponseCacheKey('summary', { articleSlug: 'post-a', lang: 'zh', queryKey: 'q1' }))
      .toBe('response:summary:post-a:zh:q1');
    expect(buildResponseCacheKey('summary', { articleSlug: 'post-b', lang: 'zh', queryKey: 'q1' }))
      .toBe('response:summary:post-b:zh:q1');
    expect(buildResponseCacheKey('summary', { articleSlug: 'post-a', lang: 'en', queryKey: 'q1' }))
      .toBe('response:summary:post-a:en:q1');
  });

  it('isolates response cache keys by query within the same type', () => {
    expect(buildResponseCacheKey('recommend', { lang: 'zh', queryKey: 'astro' }))
      .not.toBe(buildResponseCacheKey('recommend', { lang: 'zh', queryKey: 'vue' }));
  });
});
...
describe('response cache round-trip isolation', () => {
  it('does not cross-hit different queryKey values within the same type', async () => {
    ...
    const astro = await getResponseCache(cache, 'recommend', { lang: 'zh', queryKey: 'astro' });
    const vue = await getResponseCache(cache, 'recommend', { lang: 'zh', queryKey: 'vue' });
