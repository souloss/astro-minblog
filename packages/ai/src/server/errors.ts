...
export const errors = {
  methodNotAllowed: (lang?: string) =>
    chatError(
      "METHOD_NOT_ALLOWED",
      lang === "en" ? "Method not allowed" : "方法不允许",
      405
    ),
  ...
  inputTooLong: (max: number, lang?: string) =>
    chatError(
      "INPUT_TOO_LONG",
      te("ai.error.inputTooLong", lang, { max }),
      400
    ),
  rateLimited: (retryAfter?: number, lang?: string) =>
    chatError("RATE_LIMITED", te("ai.error.rateLimit", lang), 429, {
      retryable: true,
      retryAfter: retryAfter ?? 10,
    }),
  timeout: (lang?: string) =>
    chatError("TIMEOUT", te("ai.error.timeout", lang), 504, {
      retryable: true,
    }),
  providerUnavailable: (lang?: string) =>
    chatError("PROVIDER_UNAVAILABLE", te("ai.error.unavailable", lang), 503, {
      retryable: true,
      retryAfter: 30,
    }),
  internal: (detail?: string, lang?: string) =>
    chatError("INTERNAL_ERROR", detail ?? te("ai.error.generic", lang), 500, {
      retryable: true,
      retryAfter: 5,
    }),
};
